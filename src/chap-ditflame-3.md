...
この事象を回避するには、例えばですが**数千時間可動したら意図的にストレージを1本外して予備のストレージを刺し、RAIDを再構築させる**損耗率コントロールや、事前にストレージを動かしてエージング(※初期故障しそうな時間分あらかじめ稼働させてしまい、初期故障しそうなデバイスを予め検証する事)しておくさい、エージングの時間を意図的に変えて損耗状態をバラバラにする、などといった信頼性工学に基づくアプローチが必須となります。
