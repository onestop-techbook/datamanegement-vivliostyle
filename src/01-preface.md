---
class: preface
---

# まえがき

私たちの日常は、写真や動画、仕事の資料やコードなど、数えきれないほどのデータに支えられています。しかし「突然パソコンが壊れた」「クラウドが消えた」「誤って削除した」といった経験は、多くの人にとって他人事ではありません。

最近(25年10月)には、韓国でデータセンターの火災により、8年分の政府資料が消失したといったニュースがありました。細かい運用はわかりませんが、データセンターであっても絶対に大丈夫というわけではないですし、クラウド・データセンターに置いてあっても消失や停止に備えた(すくなくとも頭の片隅には置いた)対応が必要と考えた方がよさそうです。

本書は、エンジニアが知っておくべきデータマネジメントとバックアップの基本を、実体験や具体例を交えて解説するものです。HDDやクラウドといった媒体の特徴から、バックアップの3-2-1ルール、運用の工夫までを紹介し、データが失われない仕組みづくりを目指します。トラブルがあっても泣かなくてすむように、という観点で現実的な運用を目指します。大切なデータを守る第一歩として、本書を役立てていただければ幸いです。

<div class="flush-right">
企画・編集　おやかた@oyakata2438
</div>

## 対象読者

対象読者は、こんな人です。当てはまった方も、当てはまらなかった方も…

* デスクトップが散らかってる人
* HDD/SSDを吹っ飛ばした/紛失したことのある人
* データ圧縮とかをざっくり知りたい人

## リポジトリについて

この本のリポジトリは https://github.com/onestop-techbook/datamanegement-vivliostyle です。実際のソースコードを公開していますので、追記したいことなどあれば、プルリクを上げてください。増刷となった際には物理本にも反映されます。

著者щ(ﾟДﾟщ)ｶﾓｰﾝ


## 免責事項

* 本書の内容は、情報提供のみを目的としております。著者一同、正確性には留意しておりますが、正確性を保証するものではありません。この本の記載内容に基づく一切の結果について、著者、編集者とも一切の責任を負いません。
* 個別の事象についてのアドバイス等はお受けできません。あくまで著者の体験を記述しているものであり、一般に適合しない事例もあります。
* 会社名、商品名については、一般に各社の登録商標です。TM表記等については記載しておりません。また、特定の会社、製品、案件について、不当に貶める意図はありません。
* 本書の一部あるいは全部について、無断での複写・複製はお断りします

