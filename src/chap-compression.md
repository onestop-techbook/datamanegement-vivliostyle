...
また、このようにデータの偏りがある場合には有効ですが、ランダムなデータにおいては効率的な圧縮が難しい点に注意が必要です。なお、上記の例では、元の文字に合わせて区切りをしてあるので読めますが、11ビットを詰めて書くとどう読むの？ってなりそうです。ですが、ちゃんと符号化するときに重複／読み間違いをしないようにビットを設定するのでご心配なく。
...
原則としてあらゆる圧縮方式は「対象となるデータの特徴を利用する」という前提の上に成り立っています。テキストにはテキストの、画像には画像の、音声には音声特有のデータの偏りや規則性があり、それらを活用して初めて高い圧縮率が得られるのです。
...
結果として、ある圧縮アルゴリズムで圧縮したファイルは、同じ圧縮アルゴリズムでは当然として、別の圧縮アルゴリズムを利用してもほとんど圧縮することはできません。

逆に、情報の疎なファイルは簡単に圧縮することができ、また同じファイルを複数含む場合も高い圧縮度で圧縮することができます。

それを逆手に取った攻撃手法があります。
ZIPボムなどと呼ばれる攻撃で、解凍前は数MBやそれ以下の圧縮ファイルなのに、解凍するとテラバイト（1000GB）やエクサバイト(1000TB)のファイルとなり、メモリを食いつぶしてシステムをフリーズさせたりする攻撃です。ウイルスがいるわけではないため、ウイルスとしては検知されません。
...
MP3の仕組みもまた、「人間の耳の特性を利用して情報を削減する」という非可逆圧縮の典型例です。
...
非可逆圧縮とは、人間の視覚・聴覚特性に基づいて「重要でない情報」を削り、劇的な圧縮率を得る技術です。JPEG や MP3 はその代表例であり、周波数成分の分解、量子化、人間の視聴覚特性モデルの利用などの工夫によって高い圧縮率を達成しています。今日のインターネットやマルチメディアは、これら非可逆圧縮技術なしには成立しません。

<div class="column">
<div class="column-title">音声通話やビデオ通話時のパケットロスにも似た話が</div>

実は、非可逆圧縮と似たような論点の話に、音声通話やビデオ通話時のパケットロスにも似た話があります。

リアルタイムでの通話時の通信については、仮にパケットロスが発生してもその前後の情報からロスしたパケットの内容を補完（要は前後の音声波形や動画の画像の平均の値を取るように）し、パケットロスを感じさせないようにする技術が一般的に使われているのですが、これは本質的には非可逆圧縮で起きている圧縮手法と同じ事が起きていると言えるのです。
